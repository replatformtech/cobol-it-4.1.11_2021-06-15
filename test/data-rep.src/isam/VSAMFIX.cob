      ...
      *    ENVIRONMENT-VALUE IS ENVIRONMENT-VALUE          
       ...
       FILE-CONTROL.
           SELECT VSAMFILE  ASSIGN TO EXTERNAL VSAMFIXDAT
      ...
      *       END-ACCEPT
             ...
             IF 1-KEY
                  DISPLAY "VSAM FIXED on primary key" 
                  
             ELSE
                  DISPLAY "VSAM FIXED on alternate key" 
             END-IF
             PERFORM DISPLAY-RESULT
             MOVE all '-' to R-MSG
             PERFORM DISPLAY-RESULT
      ...
      * Equal a part [unsupported WITH SIZE clause in COBOL-IT]             
